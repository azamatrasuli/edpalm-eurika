...
export function ChannelPieChart({ channels }) {
  if (!channels || Object.keys(channels).length === 0) {
    return <p className="text-[13px] text-[var(--text-secondary,#6b7280)] py-4">Нет данных</p>
  }
  ...
  return (
    <div className="rounded-xl border border-[var(--border-subtle,#e5e7eb)] bg-[var(--bg-primary,#fff)] p-4">
      <h3 className="text-sm font-medium text-[var(--text-primary,#111)] mb-3">Каналы</h3>
        ...
        <div className="flex flex-col gap-2">
          {data.map((d, i) => (
            <div key={d.name} className="flex items-center gap-2 text-[13px]">
              ...
              <span className="text-[var(--text-primary,#111)]">{d.name}: {d.value}</span>
            </div>
          ))}
        </div>
      </div>
